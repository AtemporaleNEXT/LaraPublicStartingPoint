...
class NewPublicLayout extends Command
{
    ...
    protected $signature = 'make:publicLayout {name}';

    protected $description = 'Make a new public layout';
    ...
    public function handle()
    {
        ...
        if($this->confirm('This command will generates a new layout. Are you sure?')){
            ...
            $indexJade = @fopen("./resources/assets/jade/template/$name.jade",'a');
            $contenutoIndex =<<<end
include ./mixin.jade
doctype html
html(class="no-js", lang="pt-br")
    ...
    +yield('content')
            ...
            $newLess = @fopen("./resources/assets/less/components/$name.less",'a');
            @touch($newLess);// crea LESS vuoto
            //fine file less/pages/__new__.less
            $newJS = './resources/assets/js/components/$name.js';
            @touch($newJS);// crea JS vuoto
            //inizio crea cartella public img
            $makeDirIMG ='./public/img/layout/$name';
